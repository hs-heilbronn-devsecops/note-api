...
from note_api.main import create_note, get_note, get_notes, update_note
...
def test_update_and_get_item():
    backend = MemoryBackend()
    id = create_note(CreateNoteRequest(
        title='Test Note',
        description='Demo Note',
    ), backend)
    update_note(id, CreateNoteRequest(
        title='Updated Test Note',
        description='Updated Demo Note',
    ), backend)
    assert get_note(id, backend) == Note(title='Updated Test Note', description='Updated Demo Note', id=id)
